...
export const Route = createFileRoute('/team/$teamId/links/category/$categoryId')({
  component: CategoryLinksPage,
})

function CategoryLinksPage() {
  const { teams, currentTeam } = useAuthContext()
  const { teamId, categoryId } = Route.useParams()
  ...
  const { data: categoryData } = useQuery({
    ...
    queryFn: () => serverGetCategories({ data: { teamId } }).then(result => 
      result.data?.find((cat: any) => cat.id === categoryId)
    ),
    staleTime: 5 * 60 * 1000,
  })
  ...
  }), [teamId, categoryId, isSearchActive, debouncedSearchQuery, filters])

  // Fetch category links
  const { data: linksData, isLoading, isFetching } = useQuery({
    queryKey: ['links', teamId, 'category', categoryId, useSearch, searchParams],
    queryFn: () => {
      // If search or filters are active, use search function
      if (useSearch) {
        return serverSearchLinks({ data: searchParams })
      }
      // Otherwise use regular query for category links
      return serverGetLinksByCategory({ data: { teamId, categoryId } })
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  })
    ...
    try {
      switch (action) {
        ...
        case 'export':
          ...
          a.href = url
          a.download = `${category.name}-links-export.json`
          a.click()
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                {category.description || `Links in the ${category.name} category`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <form onSubmit={handleSearch} className="relative">
              ...
              <Input
                placeholder={`Search ${category.name} links...`}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full py-12">
            ...
            >
              ...
              <h3 className="text-xl font-semibold mb-2">No links found in {category.name}</h3>
              <p className="text-muted-foreground mb-6 max-w-md">
                {useSearch
                  ? `No links in ${category.name} match your search criteria. Try adjusting your search or filters.`
                  : `Add links to the ${category.name} category to get started`
                }
              </p>
              {!useSearch && (
