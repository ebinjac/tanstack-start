...
export const Route = createFileRoute('/team/$teamId/links/application/$applicationId')({
...
function ApplicationLinksPage() {
  ...
  const { data: applicationData } = useQuery({
    ...
    queryFn: async () => {
      const result = await serverGetApplications({ data: { teamId } })
      return result.data?.find((app: any) => app.id === applicationId)
    },
    staleTime: 5 * 60 * 1000,
  })
  ...
  const searchParams = useMemo(() => ({
    ...
    ...(filters.isPinned !== 'all' && { isPinned: filters.isPinned === 'true' }),
    sortBy: filters.sortBy as 'createdAt' | 'updatedAt' | 'title' | 'clickCount' | 'lastAccessedAt',
    sortOrder: filters.sortOrder as 'asc' | 'desc'
  }), [teamId, applicationId, isSearchActive, debouncedSearchQuery, filters])

  // Fetch links
  const { data: linksData, isLoading, isFetching } = useQuery({
    queryKey: ['links', teamId, 'application', applicationId, useSearch, searchParams],
    queryFn: () => {
      // If search or filters are active, use search function
      if (useSearch) {
        return serverSearchLinks({ data: searchParams })
      }
      // Otherwise use regular query for application links
      return serverGetLinksByApplication({ data: { teamId, applicationId } })
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  })
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <Button className="mt-4" asChild>
              <Link to="/team/$teamId/links/all-links" params={{ teamId }}>
  ...
  const handleSelectAll = (selected: boolean) => {
    if (selected) {
      setSelectedLinks((links as any[]).map((link: any) => link.id))
    } else {
      setSelectedLinks([])
    }
  }
    ...
    try {
      switch (action) {
        case 'delete':
          ...
          return // Don't continue, wait for dialog confirmation
        case 'pin':
        ...
        case 'archive':
          ...
          break
        case 'export':
          const selectedLinkData = (links as any[]).filter((link: any) => linkIds.includes(link.id))
          const exportData = JSON.stringify(selectedLinkData, null, 2)
          const blob = new Blob([exportData], { type: 'application/json' })
          const url = URL.createObjectURL(blob)
          const a = document.createElement('a')
          a.href = url
          a.download = `${application.tla || application.applicationName}-links-export.json`
          a.click()
  ...
  const handleBulkDelete = async () => {
    try {
      await serverBulkDeleteLinks({ data: { linkIds: selectedLinks } })
      queryClient.invalidateQueries({ queryKey: ['links', teamId] })
      setSelectedLinks([])
      setShowBulkDeleteDialog(false)
      toast.success(`${selectedLinks.length} link(s) deleted successfully`)
    } catch (error) {
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-2">
                ...
                {application.tla || application.applicationName} Links
              </h1>
              <p className="text-muted-foreground mt-1">
                Links for the {application.applicationName} application
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <form onSubmit={handleSearch} className="relative">
              ...
              <Input
                placeholder={`Search ${application.tla || application.applicationName} links...`}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={filters.isPinned} onValueChange={(value) => handleFilterChange('isPinned', value)}>
                        <SelectTrigger className="h-9">
                          <SelectValue placeholder="All" />
                        </SelectTrigger>
                        <SelectContent>
            ...
            {/* Add Link Button */}
            <Button onClick={() => setEditingLink({ applicationId })}>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full py-12">
            ...
            >
              ...
              <h3 className="text-xl font-semibold mb-2">No links found for {application.tla || application.applicationName}</h3>
              <p className="text-muted-foreground mb-6 max-w-md">
                {useSearch
                  ? `No links for ${application.tla || application.applicationName} match your search criteria. Try adjusting your search or filters.`
                  : `Add links to the ${application.tla || application.applicationName} application to get started`
                }
              </p>
              {!useSearch && (
                <Button onClick={() => setEditingLink({ applicationId })} size="lg">
                  <Plus className="h-4 w-4 mr-2" />
                  Create Link
                </Button>
              )}
              ...
              >
                ...
                <LinkCard
                  link={{
                    ...
                    applicationTla: application?.tla,
      ...
      <AlertDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {selectedLinks.length} link(s)?</AlertDialogTitle>
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the selected {selectedLinks.length} link(s).
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
